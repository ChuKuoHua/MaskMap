...
function CurrentlyPosition(position) {
  ...
  L.marker([my_Lat, my_Long], { icon: redIcon })
    .addTo(map)
    .bindPopup(`目前所在位置`)
    .openPopup()
}
...
function mask_Data() {
  ...
  mask_xhr.open(
    'get',
    'https://raw.githubusercontent.com/kiang/pharmacies/master/json/points.json?fbclid=IwAR17R1VInHOUDbahpcbn9XA4Q7o-CTIScJS-6uEZVWT0Wpnf_ictj5YDZ-w',
    true
  )
  mask_xhr.send(null)
  mask_xhr.addEventListener('load', function () {
    ...
    if (mask_xhr.status == 200) {
      ...
      for (let c = 0; getData.features.length > c; c++) {
        ...
        //地圖藥局資料顯示
        markers.addLayer(
          L.marker([coordinates1, coordinates2], { icon: maskColor })
            .bindPopup(`
                    <h1>${pharmacy}</h1>
                    <p>
...
function searchData() {
  ...
  const searchResult = newlist.filter(function (value) {
    if (
      value.properties.address.indexOf(searchStr) !== -1 ||
      value.properties.name.indexOf(searchStr) !== -1
    ) {
      return searchStr
    } else {
      return false
  ...
  function searchDataUpdate(searchValue, searchData) {
    ...
    for (let i = 0; i < searchData.length; i++) {
      ...
      switch (searchValue) {
        ...
        default:
          if (
            searchData[i].properties.mask_adult !== 0 ||
            searchData[i].properties.mask_child !== 0
          ) {
            maskNum++
          }
          condition++
...
function changeMapCenter(str) {
  ...
  L.marker([map_lat, map_long], { icon: maskColor })
    .addTo(map)
    .bindPopup(
      `
        <h1>${pharmacy}</h1>
  ...
  function filterDataUpdate(filterValue, UpdateData) {
    ...
    for (let i = 0; i < UpdateData.length; i++) {
      ...
      switch (filterValue) {
        ...
        default:
          if (
            UpdateData[i].properties.mask_adult !== 0 ||
            UpdateData[i].properties.mask_child !== 0
          ) {
            maskNum++
          }
          condition++
...
function selectMapCenter(str) {
  ...
  //點選藥局後，地圖移置中心點並顯示該藥局資訊
  L.marker([select_map_lat, select_map_long], { icon: maskColor })
    .addTo(map)
    .bindPopup(
      `
        <h1>${pharmacy}</h1>
    ...
    .openPopup()
  map.setView(new L.LatLng(select_map_lat, select_map_long), 26, {
    animate: true
  })
}
